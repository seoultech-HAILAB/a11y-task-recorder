...
3. 불편한 순간에는 `NVDA+Control+I`를 누릅니다. 높은 확인음이 납니다.
...
기록 상태는 언제든 `NVDA+Control+L`로 확인할 수 있습니다(상승음:
...
NVDA 재시작 후 `NVDA+Control+L`을 누릅니다.
...
6. 불편한 순간 `NVDA+Control+I`를 누릅니다.
...
- step별 수행 결과(성공 · 힌트 후 성공 · 막힘)와 사유 기록. step 종료 시 힌트
  연결 여부로 기본값을 채우고 회고에서 수정
- 평가 중 불편 지점 마커. 대시보드에서 표시하면 강도 1~5를 함께 기록
- **NVDA 애드온 연결 감시**: 애드온의 실제 통신을 서버가 확인해 대시보드에
  연결 상태를 표시하고, 미연결 상태로 기록이 시작되지 않도록 경고. 키트
  실행 시에는 NVDA를 자동 재시작해 복구
- 종료 후 타임라인 검토: 불편·힌트 지점 빠른 이동, 선택 구간 시각화,
  상호작용 단위 · 원시 이벤트 보기 전환
- 세션별 수집 품질 자동 점검(`data_quality`)
- 이벤트 구간과 step 구간에 문제 설명, 심각도, 기대 발화와 태그 연결
- 사용자 종합 의견 저장
- 전체 세션 JSON(원본 이벤트 + 행동 단위 해석본), 이벤트 CSV, 행동 CSV 내보내기
- 완료 세션 전체를 **결과 패키지**(세션별 JSON·CSV + 복구용 전체 기록 백업이 담긴 ZIP 하나)로
  일괄 내보내기
...
5. 불편을 느낀 순간 `NVDA+Control+I`를 누릅니다. 높은 확인음이 납니다.
...
8. 문제 설명과 기대 발화를 등록하고 종합 의견을 저장한 뒤 JSON 또는 CSV로
   ...
   **결과 패키지 만들기**로 완료 세션 전체와 복구용 전체 기록 백업이 담긴 ZIP
   하나를 만들어 보냅니다.
9. 같은 과업을 반복 측정하려면 완료된 세션의 **+ 다음 회차**를 누릅니다.
   과업 설정과 step 구성이 복사된 새 회차가 만들어지고, 세션 화면 상단의
   회차 탭(1회차·2회차…)으로 각 회차 결과를 오가며 볼 수 있습니다.

`NVDA+Control+L`을 누르면 애드온 기록 상태를 소리로 확인할 수 있습니다.
상승음은 기록 중, 낮은음은 대기 중을 뜻합니다.

`I`는 Issue(불편 지점), `L`은 Logger(기록 상태)를 뜻합니다. NVDA 기본 명령과의
충돌을 피하기 위해 `NVDA+Control+M/R`은 사용하지 않습니다. 이전 버전의
`NVDA+Control+Shift+M/R`도 호환 단축키로 계속 동작합니다.
...
- 전달: 대시보드 기록 보관함의 **결과 패키지 만들기**를 누르면 완료 세션
  전체의 JSON·CSV와 복구용 전체 기록 백업(`recorder.sqlite3`)이 `결과`
  폴더에 ZIP 하나로 만들어집니다. 이 파일을 연구 담당자에게 보내면 됩니다.
...
### NVDA 로그가 들어오지 않음 (키 입력·발화가 기록되지 않음)

가장 흔한 원인은 **NVDA는 실행 중인데 애드온이 로드되지 않은 상태**입니다.
NVDA가 로그온 화면에서 올라왔거나 비정상 종료 후 복구된 경우에 발생하며,
프로세스만 보면 정상으로 보이기 때문에 놓치기 쉽습니다.

1. 대시보드 상단의 **NVDA 연결됨 / 미연결** 표시를 확인합니다. 미연결이면
   기록해도 페이지 이동만 남고 키 입력과 발화는 저장되지 않습니다.
2. **NVDA를 재시작**합니다(대부분 이것으로 복구됩니다). 키트에서는
   `평가시작.bat`이 연결을 확인하고 자동으로 재시작을 시도합니다.
3. `NVDA+Control+L`로 기록 상태음을 확인합니다.
4. 그래도 안 되면 NVDA 메뉴 → 도구 → 애드온 스토어에서 애드온이 활성 상태인지,
   설치 후 NVDA를 재시작했는지 확인합니다.
5. 서버 상태는 `http://127.0.0.1:8765/api/health`의 `nvda_connected` 값으로도
   직접 확인할 수 있습니다.

이미 진행한 세션의 NVDA 데이터는 복구할 수 없으므로, 미연결로 기록된 세션은
다시 평가해야 합니다.
...
- 기록 시작 전 대시보드 상단에 **NVDA 연결됨**이 표시되는가
- 세션 시작 후 2초 안에 `NVDA+Control+L`이 상승음을 내는가
...
- `NVDA+Control+I` 마커가 현재 포커스 맥락과 함께 나타나는가
