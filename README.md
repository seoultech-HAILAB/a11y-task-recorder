...
### 3. NVDA 애드온 설치 — 처음 한 번

1. 압축을 푼 폴더의 `a11yTaskRecorder-0.2.0.nvda-addon` 파일을 더블클릭합니다.
2. NVDA가 설치를 물으면 승인하고, 안내에 따라 NVDA를 재시작합니다.

### 4. 평가 시작 — 평가할 때마다

1. 압축을 푼 폴더의 `평가시작.bat`을 더블클릭합니다. 처음 실행할 때 보안
   경고가 나오면 **추가 정보 → 실행**을 누릅니다.
2. 검은색 서버 창과 평가용 브라우저(기록 대시보드)가 자동으로 열립니다.
