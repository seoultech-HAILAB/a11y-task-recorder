...
| 출처 | 기록 내용 |
| --- | --- |
| NVDA | 누른 키(예: Tab, H), 그 순간 포커스된 요소의 이름·역할(예: "로그인 · 링크"), NVDA가 읽어 준 안내 문구 전문과 읽기 시작·중단 시각, 불편 표시 시점 |
| 평가용 브라우저 | 방문한 페이지 주소(검색어 등 매개변수 제외)와 이동 방향, 페이지 제목, 화면 변화 횟수(변화된 내용 자체는 저장하지 않음) |
| 연구자 입력 | 시나리오 제목·참여자 코드·step, 힌트 내용, 문제 라벨, 참여자 종합 의견 |
| 환경 정보 | NVDA 버전과 합성기, 브라우저·확장 버전 |
